...
import { buildCashierOrderSchema, type CashierOrderFormValues } from "@/components/cashier/cashierOrderSchema";
...
export function CashierCartPanel({ items, onIncrement, onDecrement, onRemove, onOrderComplete }: Props) {
  ...
  const total = useMemo(
    () => (appliedDiscount ? subtotal - (appliedDiscount / 100) * subtotal : subtotal),
  ...
  const { data: usersResult, isFetching: isSearchingUsers } = useGetAllUsersQuery(
    { searchTerm: debouncedCustomerSearch, pagination: { pageNumber: 1, pageSize: 5 } },
    { skip: customerMode !== "registered" || debouncedCustomerSearch.trim().length < 2 },
  );
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      toast.error((err as ApiError)?.message || t("cashierPos.cart.toast.couponError"));
    }
  };
  ...
  const onSubmit = async (values: CashierOrderFormValues) => {
    try {
      await addPosOrder({
        customerId: values.customerMode === "registered" ? values.customerId : undefined,
        guestName: values.customerMode === "guest" ? values.guestName?.trim() : undefined,
        guestPhone: values.customerMode === "guest" ? values.guestPhone?.trim() || undefined : undefined,
        items: items.map((i) => ({ productId: i.productId, quantity: i.quantity })),
        couponCode: appliedDiscount ? appliedCouponCode ?? undefined : undefined,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="rounded-3xl border border-border bg-card shadow-sm p-4 sm:p-5 flex flex-col h-full max-h-full">
      <h3 className="text-base font-black text-foreground mb-3">{t("cashierPos.cart.title")}</h3>

      <div className="flex-1 overflow-y-auto flex flex-col gap-2 min-h-0">
        {items.length === 0 && (
          <p className="text-xs text-muted-foreground text-center py-8">{t("cashierPos.cart.emptyCart")}</p>
        )}
        {items.map((item) => (
          <CashierCartItemRow
            key={item.productId}
      ...
      <div className="border-t border-border/60 pt-3 mt-3 flex flex-col gap-3 shrink-0">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={cn(
              ...
              customerMode === "guest" ? "bg-primary text-primary-foreground" : "bg-muted/50 text-muted-foreground",
            )}
          >
          ...
          <button
            ...
            className={cn(
              "flex-1 py-1.5 rounded-xl text-xs font-bold transition-colors",
              customerMode === "registered" ? "bg-primary text-primary-foreground" : "bg-muted/50 text-muted-foreground",
            )}
          >
        ...
        {customerMode === "guest" ? (
          <div className="flex flex-col gap-1">
            ...
            />
            {errors.guestName && <p className="text-[11px] text-rose-500">{errors.guestName.message}</p>}
            ...
            />
            {errors.guestPhone && <p className="text-[11px] text-rose-500">{errors.guestPhone.message}</p>}
          </div>
        ) : (
          <Controller
            control={control}
            ...
            render={({ field }) => (
              <div className="relative">
                <input
                  ...
                  className="w-full text-xs rounded-xl border border-border bg-background px-3 py-2 outline-none focus:border-primary"
                />
                {customerSearch.trim().length >= 2 && !field.value && (
                  <div className="absolute z-10 top-full mt-1 w-full bg-card border border-border rounded-xl shadow-lg max-h-40 overflow-y-auto">
                    {isSearchingUsers && (
                      <p className="text-xs text-muted-foreground p-2">{t("cashierPos.cart.searching")}</p>
                    )}
                    {usersResult?.data?.length === 0 && !isSearchingUsers && (
                      <p className="text-xs text-muted-foreground p-2">{t("cashierPos.cart.noCustomersFound")}</p>
                    )}
                    {usersResult?.data?.map((u) => (
                      ...
                      >
                        {u.fullName || u.userName} {u.phoneNumber ? `— ${u.phoneNumber}` : ""}
                      </button>
                    ))}
                  </div>
                )}
        ...
        {appliedDiscount != null && (
          <p className="text-[11px] text-emerald-600 font-bold">
            {t("cashierPos.cart.couponApplied", { discount: appliedDiscount })}
          </p>
        )}
        ...
        <div className="flex flex-col gap-1 text-xs">
          <div className="flex justify-between text-muted-foreground">
            ...
            <span>{subtotal.toLocaleString()} {t("cashierPos.currency")}</span>
          </div>
          {appliedDiscount != null && (
            <div className="flex justify-between text-emerald-600">
              <span>{t("cashierPos.cart.discount")}</span>
              <span>- {(subtotal - total).toLocaleString()} {t("cashierPos.currency")}</span>
            </div>
          )}
          <div className="flex justify-between text-base font-black text-foreground pt-1 border-t border-border/60">
            ...
            <span>{total.toLocaleString()} {t("cashierPos.currency")}</span>
          </div>
        </div>

        {/* المبلغ المدفوع */}
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-muted-foreground">{t("cashierPos.cart.amountPaidLabel")}</label>
          <input
            type="number"
            inputMode="decimal"
            {...register("amountPaid")}
        ...
        <div
          className={cn(
            ...
            change < 0 ? "bg-rose-500/10 text-rose-600" : "bg-emerald-500/10 text-emerald-600",
          )}
        >
          <span>{change < 0 ? t("cashierPos.cart.amountShort") : t("cashierPos.cart.changeDue")}</span>
          <span>{Math.abs(change).toLocaleString()} {t("cashierPos.currency")}</span>
        </div>
        ...
        >
          {isSubmitting ? t("cashierPos.cart.submitting") : t("cashierPos.cart.submitBtn")}
        </button>
      </div>
    </form>
  );
